...
class Graph:
    ...
    def add_bag(self, bag: str):
        self.bags.add(bag)

    # parent points to child with weight weight
    def add_edge(self, parent: str, child: str, weight: int):
        if parent not in self.bags:
            self.add_bag(parent)

        if child not in self.bags:
            self.add_bag(child)

        self.children[parent][child] = weight
        self.parents[child][parent] = weight

    def get_parents_of(self, bag):
        return self.parents[bag] if bag in self.bags else {}

    def get_children_of(self, bag):
        return self.children[bag] if bag in self.bags else {}

    def get_root_nodes(self):
        return [color for color in self.bags if not self.parents[color]]


CONTAINED_BAG_REGEX = re.compile(r'(\d+) (\w+ \w+) bag|bags\.?')


def parse_bag(line: str, graph: Graph):
    containing_bag, contained_bags = line.split(' bags contain ')
    graph.add_bag(containing_bag)
    for s in contained_bags.split(', '):
        match = CONTAINED_BAG_REGEX.match(s)
        if match:
            graph.add_edge(containing_bag, match.group(2), int(match.group(1)))


# path is just helpful visualizing examples
def dfs(graph, color, target, path):
    if color == target:
        # print(f'path = {path + [target]}')
        return 1
    if not graph.get_children_of(color):
        return 0
    if any(dfs(graph, c, target, path + [color]) for c, weight in graph.get_children_of(color).items()):
        return 1
    return 0


def dfs_count(graph, color):
    if not graph.get_children_of(color):
        return 0
    # this bag contains w bags of color c, plus the w * dfs_count() bags contained in bag c
    return sum(dfs_count(graph, c) * w + w for c, w in graph.get_children_of(color).items())


if __name__ == '__main__':
    graph = Graph()
    for line in rules.splitlines():
        parse_bag(line, graph)

    # part 1
    ways = 0
    for color in graph.bags:
        if color == 'shiny gold':
            continue
        ways += dfs(graph, color, 'shiny gold', [])
    print(ways)

    # part 2
    print(dfs_count(graph, 'shiny gold'))
