...
passports = """ecl:hzl byr:1926 iyr:2010
...
# part 1 just validate that the following fields are present:
# byr (Birth Year)
# iyr (Issue Year)
# eyr (Expiration Year)
# hgt (Height)
...
def is_valid_field(passport_field: str) -> bool:
    key, value = passport_field.split(':')
    if key == 'byr':
        return len(value) == 4 and 2002 >= int(value) >= 1920
    elif key == 'iyr':
        return len(value) == 4 and 2020 >= int(value) >= 2010
    elif key == 'eyr':
        return len(value) == 4 and 2030 >= int(value) >= 2020
    elif key == 'hgt':
        ...
        return bool(PASSPORT_ID_REGEX.match(value))
    elif key == 'cid':
        return True


def get_passports(lines: str) -> List[List[str]]:
    regex = re.compile(r"\s")
    return [regex.split(s) for s in lines.split("\n\n")]


def has_required_fields(passport: List[str]) -> bool:
    present_fields = set(f.split(':')[0] for f in passport)
    return present_fields.issuperset(REQUIRED_FIELDS)
...
if __name__ == '__main__':
    ...
    valid_passports = 0
    for passport in get_passports(passports):
        if has_required_fields(passport) and all(is_valid_field(f) for f in passport):
            valid_passports += 1
    print(valid_passports)
