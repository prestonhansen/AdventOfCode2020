...
if __name__ == '__main__':
    ...
    for group in customs_forms.split("\n\n"):
        questions_answered += len(set(group.replace("\n", "")))
    print(questions_answered)

    # part 2 - only count questions where everyone answered yes
    questions_answered_yes_by_everyone = 0
    for group in customs_forms.split("\n\n"):
        questions_answered_by_each_person = list(set(person) for person in group.splitlines())
        questions_answered_yes_by_everyone += len(set.intersection(*questions_answered_by_each_person))
    print(questions_answered_yes_by_everyone)
