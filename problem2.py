...
password_specs = """9-10 b: bbktbbbxhfbpb
2-10 x: xxnxxxwxxsx
5-7 w: ghwwdrr
4-6 z: nzzjzk
7-8 s: szsssswfs
...
def parse_password_and_requirements(line: str) -> Tuple[int, int, str, str]:
...
if __name__ == '__main__':
    valid_passwords = 0
    for line in password_specs.splitlines():
        _min, _max, char, password = parse_password_and_requirements(line)
        occurrences = password.count(char)
        if _max >= occurrences >= _min:
            valid_passwords += 1
    print(valid_passwords)

    # part 2, the spec says that in e.g. 1-3 a: abcde, 1 and 3 are indices (not zero indexed!)
    # and that the character a must at exactly one of these indices
    valid_passwords = 0
    for line in password_specs.splitlines():
        i, j, char, password = parse_password_and_requirements(line)
        # xor because exactly one should be true
